...
unrelated top-level row. `--no-parent` is only for unrelated,
independent work with no parent context, never a review or repair of an
existing candidate. When a
worktree was created with the wrong lineage, correct it in place with
