...
3. `list_models` — query models only for the selected provider and confirm the profile's configured model is available. Never strip an unset model or launch a provider default. An unavailable or exhausted model pauses affected work pending user decision; never substitute another model automatically, and never fall back silently.
...
6. `create_agent` — create the agent with provider string `${provider}/${model}` and the profile's settings (modeId, thinkingOptionId). Pass a compact brief: task, scope identity per [Shared routing](routing.md) (approved spec revision for substantial new work, snapshotted small-change intent for small new work, maintenance snapshot for adopted PRs, linked intent for peer review), current candidate revision, and the profile's role notes. Never launch native GPT children pretending to be Opus: the Opus reviewer must be a claude session running the Opus model.
